...
class ControlMainWindow(QtGui.QMainWindow):
    def __init__(self, parent=None):
        ...
        #self.ui.but_soc1_on.clicked.connect(lambda: self.switch_socket(1,'on')) 
    ...
    def get_obj_type(self, id):
        ...
        cursor = db.cursor()
        # get values for item
        cursor.execute("SELECT type FROM objects WHERE id = ?", (str(id),))
    ...
    def get_all_objects(self):
        ...
        cursor.execute("SELECT id, objectID, mod FROM mods")
    ...
    def create_new_object(self,data):
        ...
        db=sqlite3.connect('budspeech.db')
        cursor = db.cursor()
        cursor.execute("INSERT INTO objects(type,room,location,how) VALUES(?,?,?,?)", (data[0],data[1],data[2],data[3]))
        db.commit()
        id=cursor.lastrowid
        for task in data[4].split(', '):
            cursor.execute("INSERT INTO mods(objectID,mod) VALUES(?,?)", (id,task))
        db.commit()
        # close DB
        db.close()
        self.update_obj_items()
    ...
    def change_object(self,curr):
        ...
        cursor = db.cursor()
        # get values for item
        cursor.execute("SELECT  type, room, location, how FROM objects WHERE id = ?", (str(id),))
        obj_data = cursor.fetchall()[0]
        cursor.execute("SELECT mod FROM mods WHERE objectID=?", (str(id),))
        all_tasks = cursor.fetchall()
        tasks=''
        for task in all_tasks:
            tasks=tasks+task[0]+', '
    ...
    def delete_object(self,id):
        ...
        cursor.execute("DELETE FROM mods WHERE objectID = ?", (id,))
        db.commit()
        self.update_obj_items()
        db.close()
        self.uicw.close()
    ...
    def update_object(self,data):
        ...
        db=sqlite3.connect('budspeech.db')
        cursor = db.cursor()
        cursor.execute("UPDATE objects SET type = ?, room = ?, location = ?, how = ? WHERE id = ?", (data[1],data[2],data[3],data[4],data[0]))
        db.commit()
        # drop all tasks 
        cursor.execute("DELETE FROM mods WHERE objectID = ?", (data[0],))
        db.commit()
        for task in data[5].split(', '):
            cursor.execute("INSERT INTO mods(objectID,mod) VALUES(?,?)", (data[0],task))
        db.commit()
        # close DB
        db.close()
        self.update_obj_items()
    ...
    def change_socket(self,curr):
        ...
        cursor = db.cursor()
        # get values for item
        cursor.execute("SELECT  objectID, IP FROM sockets WHERE id = ?", (str(id),))
    ...
    def update_socket(self,data):
        ...
        db=sqlite3.connect('budspeech.db')
        cursor = db.cursor()
        cursor.execute("UPDATE sockets SET objectID = ?, IP = ? WHERE id = ?", (data[1],data[2],data[0]))
        db.commit()
        # close DB
        db.close()
        self.update_soc_items()
    ...
    def delete_socket(self,id):
        ...
        db=sqlite3.connect('budspeech.db')
        cursor = db.cursor()
        cursor.execute("DELETE FROM sockets WHERE id = ?", (id,))
        db.commit()
        self.update_soc_items()
        db.close()
        self.uis.close()
    ...
    def create_new_socket(self,data):
        ...
        db=sqlite3.connect('budspeech.db')
        cursor = db.cursor()
        cursor.execute("INSERT INTO sockets(objectID,IP) VALUES(?,?)", (data[0],data[1]))
        db.commit()
        # close DB
        db.close()
        self.update_soc_items()
    ...
    def send_speech(self,speech):
        try:
            fullurl = 'http://'+SERVER_IP+'/cgi-bin/send.py?speech='+str(speech)
    ...
    def get_state_socket(self,id):
        ...
        db=sqlite3.connect(path+'budspeech.db')
        cursor = db.cursor()
        cursor.execute("SELECT IP FROM sockets WHERE id = ?",(str(id),))
